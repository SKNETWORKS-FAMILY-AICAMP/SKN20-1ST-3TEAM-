...
## 1.2. Purpose
- 매월 발표되는 차량 등록 데이터를 크롤링하고 시각화하여 누구나 쉽게 차량등록 현황을 확인할 수 있는 환경 제공
- 브랜드별 고객 FAQ를 통합하여 FAQ 검색 편의성 제공 

## 1.3. Key Features Summary
- 최신 데이터 반영 (2020.08 ~ 2025.08)
- 지역별(시/도), 기간별(년-월) 필터링, 차종별 용도 비중 비교
...
### 차량등록 현황 조회
- 사용자는 **특정 기간(월별)** 을 선택해 시/도별 차량 등록 현황을 조회할 수 있다. 
...
- 차량등록 현황 조회 기능은 년/월/ 단위로 필터링을 지원해야 한다.
